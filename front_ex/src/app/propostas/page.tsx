...
export default function OrdemsDeCompra() {
  ...
  useEffect(() => {
    async function buscaDados() {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/ordensdecompra/${cliente.id}`)
      const dados = await response.json()
      setOrdemsDeCompra(dados)
    }
    buscaDados()
  }, [])
  ...
  const ordemsDeCompraTable = ordemsDeCompra.map(ordem => (
    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
      ...
      <td className="px-6 py-4">
        ...
        <p><i>Enviado em: {dataDMA(ordem.createdAt)}</i></p>
      </td>
      <td className="px-6 py-4">
        {ordem.resposta ? (
          <>
            <p><b>{ordem.resposta}</b></p>
            <p><i>Respondido em: {dataDMA(ordem.updatedAt as string)}</i></p>
          </>
        ) : (
          <i>Aguardando...</i>
        )}
  ...
  return (
    <section className="max-w-7xl mx-auto">
      <h1 className="mb-6 mt-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl lg:text-5xl dark:text-white">
        Listagem de <span className="underline underline-offset-3 decoration-8 decoration-orange-400 dark:decoration-orange-600">Minhas Ordens de Compra</span>
      </h1>
